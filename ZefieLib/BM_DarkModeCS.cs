/// From https://github.com/BlueMystical/Dark-Mode-Forms
...
namespace BlueMystic
{
	...
	public class DarkModeCS
	{
		...
		public void ThemeControl(Control control)
		{
			...
			{
				control.GetType().GetProperty("BackColor")?.SetValue(control, OScolors.Control, null);
			}
			if (control.ForeColor == SystemColors.ControlText || control.ForeColor == SystemColors.WindowText)
			{
				control.GetType().GetProperty("ForeColor")?.SetValue(control, OScolors.TextActive, null);
			}
...
*/
			...
			if (control is ComboBox combo)
			{
				...
				control.GetType().GetProperty("ButtonColor")?.SetValue(control, OScolors.Surface, null);
				combo.Invalidate();
			}
...
//				pGrid.CategorySplitterColor = OScolors.ControlLight;
		...
		public static void SetRoundBorders(Control _Control, int Radius = 10, Color? borderColor = null, int borderSize = 2, bool underlinedStyle = false)
		{
			try
			{
				...
				if (_Control != null)
				{
					_Control.GetType().GetProperty("BorderStyle")?.SetValue(_Control, BorderStyle.None, null);
